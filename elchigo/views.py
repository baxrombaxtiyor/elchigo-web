...
def orders_api(request):
    ...
    for doc in db.collection('orders').where('restaurantId', '==', rid).stream():
        ...
        if user_role == 'waiter' and o.get('waiterId', '') != uid:
            continue
        dt = _parse_dt(o.get('createdAt'))
...
def menu(request):
    ...
    for cat in db.collection('restaurants').document(rid).collection('categories').order_by('order').stream():
        cat_data = cat.to_dict(); cat_data['id'] = cat.id
        dishes = []
        ...
        cat_data['dishes'] = dishes
        categories.append(cat_data)
    return render(request, 'menu/index.html', {
        'categories': categories, 'restaurant_name': request.session.get('restaurant_name', ''),
    })
...
ROLE_LABELS = {
    'waiter': 'Официант', 'manager': 'Менеджер',
    'cook': 'Ошпаз', 'cashier': 'Кассир', 'admin': 'Администратор',
}
...
def staff_api(request):
    ...
    for doc in db.collection('users').where('restaurantId', '==', rid).stream():
        ...
        rest_id = d.get('restaurantId', ''); rest_name = ''
        if rest_id:
            rest_doc = db.collection('restaurants').document(rest_id).get()
            if rest_doc.exists: rest_name = rest_doc.to_dict().get('name', '')
        users.append({'uid': doc.id, 'name': d.get('name', ''), 'email': d.get('email', ''),
                      'phone': d.get('phone', ''), 'role': role, 'restaurantId': rest_id,
                      'restaurantName': rest_name, 'blocked': d.get('blocked', False)})
...
def waiter_report(request):
    return render(request, 'reports/waiter_report.html', {
        'restaurant_name': request.session.get('restaurant_name', ''),
    })
...
def z_report(request):
    return render(request, 'reports/z_report.html', {
        'restaurant_name': request.session.get('restaurant_name', ''),
    })
